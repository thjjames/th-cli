...
const updateTemplateByAnswers = (answers, targetDir) => {
  ...
  try {
    ...
    bundlerConfig = bundlerConfig.replace(/AntDesignVueResolver/g, 'VantResolver').replace(/VantResolver\([\s\S]*?\)/g, 'VantResolver()');
    ...
    const postcssConfigPath = path.resolve(targetDir, 'postcss.config.ts');
