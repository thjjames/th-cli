...
const { getTemplateDirectories, getDefaultAnswersByPrompts, updateTemplateByAnswers } = require('./utils');
...
const prompts = [
  ...
  },
  {
    ...
    default: false,
    when: function(answers) {
      return answers.type === 'preset' && answers.preset.includes('vue3');
    }
  },
  {
    name: 'description',
    message: '请输入项目描述',
    default: `a project created by th-cli@${cliVersion}`
  },
  {
    name: 'author',
    message: '请输入作者名称',
...
const copy = (templateDir, targetDir, options) => {
  // fse.copySync(templateDir, targetDir);
  return new Promise(async (resolve, reject) => {
    await editor.copyTplAsync(templateDir, targetDir, options, {}, {
      globOptions: {
...
const create = async (name, options, targetDir) => {
  ...
  try {
    if (answers.type === 'preset') {
      await copy(templateDir, targetDir, { name, cliVersion, answers });
    } else {
    ...
    updateTemplateByAnswers(answers, targetDir);
    execSync('git init', {
      cwd: targetDir
    });
